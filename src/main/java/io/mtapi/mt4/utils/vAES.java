...
/**
 * Custom AES implementation used by the MT4 protocol for packet encryption.
 * This is the MetaTrader-specific AES variant (not standard JCE AES).
 */
public class vAES {
    private int m_nCipherRnd;
    private final int[] m_Ks = new int[64];
    private final int[] m_Ke = new int[64];
    private final int[][] s_tabIT = new int[4][256];
    private final int[][] s_tabFT = new int[4][256];
    ...
    private byte[] EncryptBlock(byte[] data) {
        ...
        w[3] &= 0xFFFFFFFFL;
        for (int j = 0; j < w.length; j++) ind[j] = (int) w[j];
        ...
        System.arraycopy(BitConverter.getBytes(SBox[ind[0] & 0xFF] ^ (SBox[(ind[1] >>> 8) & 0xFF] << 8) ^ (SBox[(ind[2] >>> 16) & 0xFF] << 16) ^ (SBox[(ind[3] >>> 24) & 0xFF] << 24) ^ m_Ks[i * 4 + 8]), 0, crypt, 0, 4);
        System.arraycopy(BitConverter.getBytes(SBox[ind[1] & 0xFF] ^ (SBox[(ind[2] >>> 8) & 0xFF] << 8) ^ (SBox[(ind[3] >>> 16) & 0xFF] << 16) ^ (SBox[(ind[0] >>> 24) & 0xFF] << 24) ^ m_Ks[i * 4 + 9]), 0, crypt, 4, 4);
        System.arraycopy(BitConverter.getBytes(SBox[ind[2] & 0xFF] ^ (SBox[(ind[3] >>> 8) & 0xFF] << 8) ^ (SBox[(ind[0] >>> 16) & 0xFF] << 16) ^ (SBox[(ind[1] >>> 24) & 0xFF] << 24) ^ m_Ks[i * 4 + 10]), 0, crypt, 8, 4);
        System.arraycopy(BitConverter.getBytes(SBox[ind[3] & 0xFF] ^ (SBox[(ind[0] >>> 8) & 0xFF] << 8) ^ (SBox[(ind[1] >>> 16) & 0xFF] << 16) ^ (SBox[(ind[2] >>> 24) & 0xFF] << 24) ^ m_Ks[i * 4 + 11]), 0, crypt, 12, 4);
        return crypt;
    }

    private long upr(long x) {
        return ((x << 8) | (x >>> (32 - 8))) & 0xFFFFFFFFL;
    }
    ...
    private int bKs(int index) {
        ...
        return switch (index % 4) {
            case 0 -> (int) (ks & 0xFF);
            case 1 -> (int) ((ks >>> 8) & 0xFF);
            case 2 -> (int) ((ks >>> 16) & 0xFF);
            case 3 -> (int) ((ks >>> 24) & 0xFF);
            default -> 0;
        };
    }
    ...
    public final byte[] EncryptData(byte[] data, byte[] key) {
    ...
    private byte[] DecryptBlock(byte[] data) {
        ...
        byte[] crypt = new byte[16];
        System.arraycopy(BitConverter.getBytes(InvSBox[(int) (ind[0] & 0xFF)] ^ (InvSBox[(int) ((ind[3] >> 8) & 0xFF)] << 8) ^
                (InvSBox[(int) ((ind[2] >> 16) & 0xFF)] << 16) ^ (InvSBox[(int) ((ind[1] >> 24) & 0xFF)] << 24) ^ m_Ke[i * 4 + 8]), 0, crypt, 0, 4);
        System.arraycopy(BitConverter.getBytes(InvSBox[(int) (ind[1] & 0xFF)] ^ (InvSBox[(int) ((ind[0] >> 8) & 0xFF)] << 8) ^
                (InvSBox[(int) ((ind[3] >> 16) & 0xFF)] << 16) ^ (InvSBox[(int) ((ind[2] >> 24) & 0xFF)] << 24) ^ m_Ke[i * 4 + 9]), 0, crypt, 4, 4);
        System.arraycopy(BitConverter.getBytes(InvSBox[(int) (ind[2] & 0xFF)] ^ (InvSBox[(int) ((ind[1] >> 8) & 0xFF)] << 8) ^
                (InvSBox[(int) ((ind[0] >> 16) & 0xFF)] << 16) ^ (InvSBox[(int) ((ind[3] >> 24) & 0xFF)] << 24) ^ m_Ke[i * 4 + 10]), 0, crypt, 8, 4);
        System.arraycopy(BitConverter.getBytes(InvSBox[(int) (ind[3] & 0xFF)] ^ (InvSBox[(int) ((ind[2] >> 8) & 0xFF)] << 8) ^
