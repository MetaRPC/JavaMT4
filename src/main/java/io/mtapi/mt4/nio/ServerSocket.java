...
public class ServerSocket {
    ...
    public byte[] readCompressed(MT4Encryption encryption) throws IOException {
        ...
        int dstLen = io.mtapi.mt4.utils.BitConverter.toInt32(buf, 0);
        buf = receiveDecoded(4, encryption);
        int srcLen = io.mtapi.mt4.utils.BitConverter.toInt32(buf, 0);
        buf = receiveDecoded(srcLen, encryption);

        byte[] result = new byte[dstLen];
        int decompressedSize = io.mtapi.mt4.utils.FastDecompressor.decompress(buf, 0, srcLen, result, dstLen);
